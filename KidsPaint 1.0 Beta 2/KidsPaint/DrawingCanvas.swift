...
struct DrawingCanvas: View {
    ...
    /// Called once at the end of a sticker manipulation gesture (move/resize/rotate)
    /// so the caller can register a single Undo step.
    ...
    /// Incrementing token for one-shot sticker placement.
    ...
    private func draw(stroke: Stroke, in context: inout GraphicsContext) {
        ...
        case .chisel:
            ...
            for i in 1..<stroke.points.count {
                ...
                var t = CGAffineTransform.identity
                t = t.translatedBy(x: p1.x, y: p1.y)
                t = t.rotated(by: angle + .pi / 6)

                context.drawLayer { layer in
                    layer.transform = t
                    layer.fill(Path(rect), with: .color(stroke.color))
                }
            }
