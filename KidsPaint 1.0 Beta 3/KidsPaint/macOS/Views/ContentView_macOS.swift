// Only compile this file for macOS
#if os(macOS)
//
//  ContentView.swift
...
struct ContentView: View {
    ...
    private var topToolbar: some ToolbarContent {
        ...
        // Left side: primary tools (match iOS split)
        SwiftUI.ToolbarItemGroup(placement: .navigation) {
            toolButton(.brush, icon: "paintbrush.fill", label: "Brush")
            toolButton(.eraser, icon: "eraser.fill", label: "Eraser")
            toolButton(.sticker, icon: "sparkles", label: "Stickers")
            toolButton(.pan, icon: "hand.draw", label: "Pan")
        }

        SwiftUI.ToolbarItemGroup(placement: .primaryAction) {
            Button {
                // Behave like Cmd-Z even if focus moved to the toolbar
        ...
        .popover(isPresented: $showMorePopover, arrowEdge: .bottom) {
            VStack(alignment: .leading, spacing: 10) {
                Button("Remove Background Image") {
                    canvasState.setBackgroundImage(nil, undoManager: capturedUndoManager)
                ...
                Button("Background: White") {
                    canvasState.setCanvasBackground(.white, undoManager: capturedUndoManager)
                    showMorePopover = false
                }

                Button("Background: Light Gray") {
                    canvasState.setCanvasBackground(Color.gray.opacity(0.15), undoManager: capturedUndoManager)
                    showMorePopover = false
                }
    ...
    private func deleteSelectedSticker() {
        ...
        canvasState.removeSticker(id: id, undoManager: capturedUndoManager)
    ...
    private func openBackgroundImage() {
        ...
        panel.begin { response in
            ...
            canvasState.setBackgroundImage(img, undoManager: capturedUndoManager)
        }
    }
